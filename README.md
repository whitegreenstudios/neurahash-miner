...
### 2026-09-04 — **The clearest result yet, and it is about YOUR card: training data generated on a remote consumer GPU over the public internet taught our model to reason AS WELL AS data generated on the machine next to the trainer. Two seeds. Parity, measured.**

**Nothing here changes how you mine.** Your miner's job is unchanged and no update is required. This
is a research result about what the work you send us is worth.

**What was measured.** A remote RTX 4060 — an 8 GB consumer card on an ordinary home connection —
generated 10,450 step-by-step maths solutions and shipped them over the internet through the public
anchor. Every slice was hash-verified on arrival. 97.5% of the solutions reached the correct final
answer. We trained on them and scored on 1,319 held-out grade-school maths problems the model had
never seen:

| | problems solved | change |
|---|---|---|
| the base model | 117 / 1319 (8.87%) | — |
| trained on your fleet's maths data | 216 / 1319 (16.38%) and 205 (15.54%) | **+7.5 and +6.7 points**, two seeds |

**The part that matters for a decentralised network:** we ran the identical recipe on data generated
locally by a much larger teacher model, and compared the two head to head on the same held-out
problems. The difference was **+0.53 points (p 0.69)** on one seed and **exactly 0.00 points
(p 1.00)** on the other. **Distance costs nothing.** Data made on a small card at the end of a home
connection taught the model exactly as well as data made beside the trainer.

**And one mixed corpus did two jobs at once.** Maths-only data improved reasoning but cost us on a
science-reasoning task. A corpus of half maths and half general instructions — every row generated by
the fleet — improved reasoning (+5.53 points on the same 1,319 problems) *and* a separate 9,547-item
commonsense battery (+1.36 points), with nothing regressing. That mix is now the default recipe. Its
second seed is training as this is published; one seed is not a claim, and we will say so either way.

**What we are NOT claiming, because you deserve the limits with the result.**
- **No gradients cross the network.** The fleet GENERATES text; one machine still does all the
  training. This is not distributed training and we will not call it that.
- **We have not shown the fleet is necessary.** Every corpus that worked came from a teacher whose
  output someone else could download, and every local comparison tied. The fleet has proved it can
  MATCH central generation, not yet that it makes something unavailable elsewhere. That is the next
  problem we are working on, and it is the honest gap in the story.
- **The model is still weak in absolute terms**: 16% on grade-school maths. AI2's own supervised
  checkpoint on the same base reaches 28%. We have closed part of a gap, not the gap.
- Roughly three quarters of the commonsense-battery gain sits in one task (BoolQ), which is true of
  every published checkpoint we tested too, including AI2's own.

**Two things we stopped doing this week, because measurement killed them.** We scored AI2's published
OLMoE checkpoints on our own frozen tests, with no training at all. The entire preference-tuning stage
is worth half a point on this instrument — below our bar — so we cancelled that track before building
its trainer. And nothing beat their plain supervised checkpoint, so we cancelled a distillation track
too. Two dead ends for 2.4 GPU-hours is the cheapest week we have had.
...
job where the machines genuinely learn different things. We ran that test. It **failed** — the combined
result scored **1.44 percentage points worse** than just using one machine on its own.

Then we checked something we should have checked first: **what does one machine score when we give it
only *half* the training data?**

It scored **exactly the same as with all of it.** Same number, to four decimal places.

That single fact reframes the whole month. Our benchmark had no room left to improve. A test like that
can still show you when combining *hurts* — and it did — but it is physically incapable of showing you
when combining *helps*, because there is no "better" left for it to point at. Every "combining does
nothing" result we published this month was measured on that same maxed-out benchmark.

**So: are we saying combining works after all?** No. We are saying **we do not know**, and we
previously told you we did. That is a downgrade in confidence, not an upgrade in results.

**The numbers, so you can hold us to them.** Frozen 3,200-question benchmark, `sha 7930be541e93...`,
untrained model scores 72.4688%.

| what we ran | score |
|---|---|
| one machine, **all** the training data | **76.2500%** |
| one machine, **half** the training data | **76.2500%** ← the problem |
| second machine, the other half | 74.1875% |
| **both machines combined** | **74.8125%** ← the failure |

The combined-vs-one-machine gap is −1.44 points, and it is not noise (p = 0.00007 on the same
questions). We also ran a control that rescales one machine's work to the same "size" as the combined
result, which splits the loss into **13% from the combining maths** and **87% from the two machines
genuinely disagreeing** about what to learn. That is useful: it means the fix is not a better averaging
formula. It is giving the two machines *different jobs* so their work does not collide.

**The fix that affects you directly: 8 GB cards were being clamped to 1 GB.**

We reserve some video memory for your desktop so mining does not freeze your screen. That reserve was a
flat 8 GB subtracted from whatever card you have. On a 32 GB card that is polite. On an **8 GB card it
leaves 1 GB**, which is not enough to do anything — so the card would accept work and then park forever,
looking healthy the whole time. This is the mechanism behind the 8 GB parking reports.

It is now proportional: the reserve can never take more than half your card. An 8 GB card goes from a
**1.00 GB budget to 6.75 GB**. If you set a memory cap yourself, it is now used exactly as you typed it
or the miner refuses to start and tells you why — it will never silently give you less than you asked
for. Eight tests cover it, including one that deliberately fails against the old formula so the test
cannot quietly stop checking.

**Two other things worth knowing.**

*Your card's software version was silently changing the experiment.* When two machines start a shared
training job they must begin from the identical random starting point. Both of ours were told to use
the same seed — but the two machines had different versions of the GPU library, and it turns out the
same seed produces **different** random numbers across versions. The two machines were starting from
unrelated places and then being asked to combine, which cannot work. Two full six-hour runs were thrown
away before we caught it. The starting point is now generated by a method that is guaranteed identical
on every machine, and there is a one-line fingerprint check that catches a mismatch in the time it
takes to load the model instead of after six hours of wasted electricity.

*We could not have rebuilt our own results.* Our main test machine was running a pre-release build of
the GPU library that has since been **deleted from the internet**. Every number we had published came
from an environment nobody — including us — could reconstruct. Both machines are now on published,
stable versions. We re-ran our best recorded result on the new stack to check nothing shifted: **76.03%
versus 76.25%**, well inside the measurement error. The record stands, and now it is reproducible.

**What we are testing right now.** A job with room to improve, by construction: 160 invented facts about
people who do not exist, so the model **cannot** already know them. It currently scores **25.0%**, which
is exactly random guessing on a four-way choice. Anything above that is knowledge that was genuinely
added rather than knowledge that was already there. We wrote the pass mark down before starting — 60%
recall, and at least a 30-point improvement, tested on a phrasing the model was never trained on.

**Unchanged:** pooled mining is still **closed** and still **not paying**, for the same reason as the
last update — we will not reopen it until we can show that miner work makes the model measurably better.
This update moves us closer to being able to answer that honestly, and no closer to claiming it.
